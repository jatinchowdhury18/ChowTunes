...
public:
    ...
    static inline void toLower(std::u8string& s)
    {
        for (size_t i = 0; i < s.size(); ++i)
            s[i] = static_cast<char8_t> ((char) std::tolower (s[i]));
    }

    static inline std::vector< std::u8string > splitString(std::u8string s)
    {
        std::vector< std::u8string > ss;
    ...
    static inline float scoreLev(const char* queryBegin, size_t queryLength, const char* wordBegin, size_t wordLength)
    {
        ...
        if (fuzzyDist >= (float) queryLength)
        {
            // late-out
            return 0;
        }
        ...
        if ((fuzzyDist * 2) > (float) wordLength)
        {
            // not good enough
            return 0;
        }
...
template <typename Key = std::u8string>
class Database
{
public:
...
private:
    ...
    struct WordStorage
    {
        ...
        std::unordered_map< std::u8string, int > _wordMap;
    ...
    static void scoreEveryWord(std::vector<float>& scores, const WordStorage& ws, const std::u8string& queryWord)
    {
        ...
        const char* queryBegin = (const char*) queryWord.c_str();
...
public:
    ...
    void addEntry(Key key, const std::vector<std::u8string>& fields)
    {
        ...
        for (size_t fieldIndex = 0; fieldIndex < fields.size(); ++fieldIndex)
        {
            ...
            {
                std::u8string& lowerWord = words[wi];
    ...
    std::vector< Result > search(std::u8string queryString) const
    {
        ...
        {
            const std::u8string& queryWord = queryWords[qi];
